...
let imgMatrix = [
  ...
  { Purity: { img: "assets/scanned vitamins/purity.jpg" } },
  { Thankful_: { img: "assets/scanned vitamins/thankful_.jpg" } },
  { Joy: { img: "assets/scanned vitamins/joy.jpg" } },
  ...
  { Compassion: { img: "assets/scanned vitamins/compassion.jpg" } },
  { Grace_: { img: "assets/scanned vitamins/grace_.jpg" } },
];
...
let resurrectionShortDaysArray = [
  "sun",
  "mon",
  "tue",
  "wed",
...
function getState() {
  //added this function to load previous series selections when returning to vitamin from letter or other screen
  ...
  if (selection == "Resurrection") {
    //set days to special if series in memory is resurrection
    displayedDay.innerHTML = resurrectionDaysArray[currentDay];
    console.log("displayedDay changed...");
    previousDisplayedDay = displayedDay.innerHTML;
    console.log(
      "the displayedDay was set to ",
      resurrectionDaysArray[currentDay]
    );
  }
  ...
  if (selection == null) {
    ...
    console.log(
      sessionStorage.getItem("storedSelection") +
...
window.onload = function () {
  ...
  if (selection == "Patriotic") {
    //set patriotic days when page first loads
    displayedDay.innerHTML = patrioticDaysArray[currentDay];
    console.log("displayedDay changed...");
    previousDisplayedDay = displayedDay.innerHTML;
    return;
    ...
    console.log("displayedDay changed...");
    console.log(
      "on window load, the displayedDay innerHTML was set to daysArray[day]",
      daysArray[day]
    );
    if (day == "0") {
      prevDay = "6";
...
jumpLinks.forEach((link) => {
  link.addEventListener("click", (event) => {
    event.preventDefault();
    console.log(
      "the link dataset name is: ",
      link.getAttribute("data-daysArray")
    );
    // if (link.getAttribute.dataset == "data-daysArray")
    if (link.innerHTML != "Home") {
      //keeps home from appearing in letter title
    ...
    if (selection == "Patriotic") {
      //set revert day using patriotic days array
      displayedDay.innerHTML = patrioticDaysArray[revertDay];
      console.log("displayedDay changed...");
      console.log(
        "the patrioticDaysArray[revertDay] is: ",
        patrioticDaysArray[revertDay]
      );
...
function prev() {
  ...
  if (scriptView.classList.contains("bonus-style")) {
    ...
    if (selection == "Patriotic") {
      //set days using patriotic days array
      ...
      //if the series does not have a specialDays dataset then run this
      if (day == "0") {
        day = 6;
        console.log("The day was 0");
        currentDay = 6;
...
viewSelection.addEventListener("click", (event) => {
  ...
  if (
    ...
    if (scriptView.classList.contains("bonus-style")) {
      console.log(
        "the scriptView className contained 'bonus-style' so it will be removed"
      );
      // displayedDay.innerHTML = previousDisplayedDay;
      // console.log("the displayedDay was set to 'previousDisplayedDay'");
      if (specialDays != "null") {
        //if the dataset is specialDays then run this
      ...
      console.log(
        "the scriptView class name did not contain bonus-style, rather: ",
        scriptView.className
      );
      if (selection == "Resurrection") {
      ...
      if (script[selection]) {
        ...
        script[selection].forEach((daily) => {
          if (daily[resurrectionShortDaysArray[currentDay]]) {
            ...
            scriptView.innerHTML =
              daily[resurrectionShortDaysArray[currentDay]];
          }
        });
      }
    });
...
scriptView.addEventListener("click", (event) => {
  //when the opened vitamin is clicked, then change to quote and then back to scripture
...
function viewChanger() {
  console.log(
    "viewChanger function running. The scriptView.innerHTML = ",
    scriptView.innerHTML
  );
  console.log("the scriptView classname is: ", scriptView.className);
  if (scriptView.classList.contains("bonus-style")) {
    ...
    if (scriptView.innerHTML === dailyScript) {
      ...
      scriptView.innerHTML === dailyQuote ||
      scriptView.innerHTML.includes("tooltiptext")
    ) {
    ...
    if (quote[selection]) {
      ...
      quote[selection].forEach((dayQuote) => {
        ...
        if (scriptView.innerHTML.includes("tooltiptext")) {
          console.log(
            "tooltiptext style is: ",
            document.querySelector(".tooltiptext").style
          );
          if (
      ...
      if (quote[selection]) {
        ...
        quote[selection].some((dayQuote) => {
          if (dayQuote[resurrectionShortDaysArray[currentDay]]) {
            ...
            scriptView.innerHTML =
              dayQuote[resurrectionShortDaysArray[currentDay]];
            console.log(
              "the quote added: ",
              dayQuote[resurrectionShortDaysArray[currentDay]],
              "the innerHTML contains: ",
              scriptView.innerHTML
            );
          }
          return;
      ...
      if (quote[selection]) {
        ...
        quote[selection].forEach((dayQuote) => {
          if (dayQuote[shortDaysArray[currentDay]]) {
            ...
            console.log(
              "the quote added: ",
              dayQuote[shortDaysArray[currentDay]],
              "the innerHTML contains: ",
              scriptView.innerHTML
            );
            //show the lyrics if tooltiptext is in the element
            if (dailyQuote.includes("tooltiptext")) {
              ...
              if (
                document.querySelector(".tooltiptext").style.visibility !=
                "visible"
              ) {
                console.log("the tooltiptext was hidden");
                document.querySelector(".tooltiptext").style.visibility =
...
function findBonusDays() {
  //function to find if vitamin series has a bonus day
...
function isThereBonus() {
  ...
  // console.log("vitamins with bonus days are: ");
  // containsBonusDay.map(function(eachName){
  containsBonusDay.some((eachName) => {
    //changed from map to some to be able to break out of loop when a match is found
    // console.log(eachName);
...
function openBonus() {
  //should run when bonus button is clicked
  console.log("the scriptView classname is: ", scriptView.className);
  ...
  scriptView.setAttribute(
    "class",
    isBonus ? "scripture" : "scripture showScripture bonus-style"
  );
  console.log("the scriptView classname is: ", scriptView.className);
  // scriptView.classList.toggle("bonus-style");
  scriptureMatrix.forEach((script) => {
  ...
  if (scriptView.classList.contains("bonus-style")) {
    if (specialDays) {
      ...
      if (specialDays == "patrioticDaysArray") {
        //set days using patriotic days array
        console.log(
          "the patrioticDaysArray[bonusDay] is ",
          patrioticDaysArray[bonusDay]
        );
        displayedDay.innerHTML = patrioticDaysArray[bonusDay];
        console.log(
          "displayedDay changed due to bonus button clicked and patriotic day vitamin selected"
        );
        return;
      }
    } else {
      //for all other bonus days run this when bonus button is clicked
